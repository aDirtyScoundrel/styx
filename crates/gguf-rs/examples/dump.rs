...
fn main() {
    ...
    for t in g.tensors.iter().take(5) {
        println!("{} dims{:?} type{} off{} size{}", t.name, t.dims, t.ggml_type, t.offset, t.size_bytes);
    }
    // expert tensors summary
    let exp: Vec<_> = g.tensors.iter().filter(|t| t.name.contains("_exps.")).collect();
    let exp_bytes: u64 = exp.iter().map(|t| t.size_bytes).sum();
    println!("expert tensors: {} totaling {:.2} GB", exp.len(), exp_bytes as f64 / 1e9);
}
